...
//! - Dynamic CLOB authentication using the SDK
...
pub fn run_onboarding_checks() -> Result<(), OnboardingError> {
    ...
    // Check 1: Private Key (required for authentication)
    check_private_key()?;
...
fn check_private_key() -> Result<(), OnboardingError> {
    match env::var("POLYMARKET_PK") {
        Err(_) => return Err(OnboardingError::MissingPrivateKey),
        Ok(val) if val.is_empty() => return Err(OnboardingError::MissingPrivateKey),
        Ok(val) if !val.starts_with("0x") => return Err(OnboardingError::InvalidPrivateKey),
        Ok(val) if val.len() < 64 => return Err(OnboardingError::InvalidPrivateKey),
        Ok(_) => {}
    }
...
fn check_database_permissions() -> Result<(), OnboardingError> {
        ...
        .open(&db_path)
    {
        ...
        Err(e) => Err(OnboardingError::DatabaseError(e.to_string())),
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            OnboardingError::MissingPrivateKey => {
                ...
                writeln!(
                    f,
                    "1. Create a file named {} in the project root folder.",
                    "'.env'".cyan()
                )?;
                writeln!(
                    f,
                ...
                writeln!(
                    f,
                    "{}",
                    "TIP: You can copy '.env.example' as a template!".yellow()
                )?;
                writeln!(f, "{}", "-".repeat(60).red())?;
                Ok(())
            }
            OnboardingError::InvalidPrivateKey => {
                writeln!(f)?;
                writeln!(
                    f,
                    "{}",
                ...
                writeln!(f, "{}", "-".repeat(60).red())?;
                writeln!(f, "The private key format is invalid.")?;
                writeln!(f)?;
                writeln!(f, "{}", ">> REQUIREMENTS:".yellow().bold())?;
                writeln!(f, "- Must start with '0x'")?;
                writeln!(f, "- Must be at least 64 characters (hex format)")?;
                writeln!(f)?;
                writeln!(f, "{}", ">> EXAMPLE:".yellow().bold())?;
                writeln!(f, "   {}", "POLYMARKET_PK=0x1234567890abcdef...".cyan())?;
                writeln!(f, "{}", "-".repeat(60).red())?;
                Ok(())
            }
            OnboardingError::DatabasePermissionDenied(path) => {
                ...
                writeln!(
                    f,
                    "{}",
                    "[!] SYSTEM ERROR: Cannot create Database File".red().bold()
                )?;
                writeln!(f, "{}", "-".repeat(60).red())?;
                writeln!(
                    f,
                    "Could not create '{}' in the current folder.",
                    path.yellow()
                )?;
                writeln!(f)?;
